...
public class GUIBlockSelection extends GuiContainer {
	...
	protected static final ResourceLocation ScrollTab = new ResourceLocation(
	...
	public void initGui() {
		...
		this.searchField = new GuiTextField(0, this.fontRendererObj, this.guiLeft + 81, this.guiTop + 6, 89,
		...
		if (showModeSwitchButton()) {
			this.modeSwitch.xPosition = (this.guiLeft + 8);
			this.modeSwitch.yPosition = this.guiTop + 109;
			this.modeSwitch.setWidth(109);
		...
		for (Block b : GameData.getBlockRegistry()) {
			if (Item.getItemFromBlock(b) != Item.getItemFromBlock(Blocks.AIR))
	...
	public void handleMouseInput() throws IOException {
		...
		int gridX = ContainerBlockSelMenu.gridX;
		int gridY = ContainerBlockSelMenu.gridY;
		
		int i = Mouse.getEventDWheel();

		if (i != 0) {
			int j = ContainerBlockSelMenu.itemList.size() / gridX - gridY;
	...
	protected void renderToolTip(ItemStack stack, int x, int y) {
		...
		if (creativetabs != null) {
			list.add(1, "" + TextFormatting.BOLD + TextFormatting.BLUE
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		this.mc.getTextureManager()
				.bindTexture(new ResourceLocation(Reference.MODID, "textures/gui/blockselectionbg.png"));
		...
		int k = j + 112;
		this.mc.getTextureManager().bindTexture(ScrollTab);
				...
				if (showMetaData.isChecked()) {
					for (ItemStack itemStack2 : blockListMeta) {
						if (itemStack.getItem() == itemStack2.getItem()
								&& itemStack.getMetadata() == itemStack2.getMetadata()) {
