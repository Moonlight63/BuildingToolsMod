	...
	*/
	...
	public IMessage onMessage(T message, MessageContext ctx) {
		...
		if (ctx.side.isClient()) {
			//System.out.print("Got to client onMessege \n");
			...
			return handleClientMessage(BuildingTools.proxy.getPlayerEntity(ctx), message, ctx);
		} else {
			//System.out.print("Got to server onMessege \n");
			// server side proxy will return the server side EntityPlayer
			return handleServerMessage(BuildingTools.proxy.getPlayerEntity(ctx), message, ctx);
		}
		//return null;
	}
}
